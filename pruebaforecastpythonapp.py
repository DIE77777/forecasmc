...
option = st.sidebar.selectbox('Select the option',('MARCA DE CANAL','TEAM SOLUTIONS'))
...
if uploaded_file is not None and run == True:
    ...
    df['producto'] = df['COD. PRODUCTO'].astype(str) + df['COD CLIENTE'].astype(str) + df['SEGMENTO'].astype(str)
    ...
    df_export_wide = df_export.pivot_table(index=['UN', 'COD. PRODUCTO', 'PRODUCTO', 'SEGMENTO', 'COD CLIENTE', 'CLIENTE'], columns='ds', values='VENTA NETA (TON)')
    ...
    df_export_wide['key'] = df_export_wide['COD. PRODUCTO'].astype(str) + df_export_wide['COD CLIENTE'].astype(str) + df_export_wide['SEGMENTO'].astype(str)
    ...
    for i in df_export_wide.columns[-13:]:
      df_export_wide[i] = np.where(df_export_wide['COD. PRODUCTO'].isin(product_filter), df_export_wide['avg'], df_export_wide[i])
    ...
    apertura_secuencia['VENTA NETA (TON)'] = apertura_secuencia['VENTA NETA (TON)'].apply(lambda x: 0 if x < 0 else x)

    # create a key to join the df_export_wide with the apertura_secuencia dataframe

    apertura_secuencia['key'] = apertura_secuencia['COD. PRODUCTO'].astype(str) + apertura_secuencia['COD CLIENTE'].astype(str) + apertura_secuencia['SEGMENTO'].astype(str)

    # fill the nan values with 0

    apertura_secuencia.fillna(0, inplace=True)

    # select the columns with forecast values and keys in df_export_wide

    df_export_wide_secuencia = df_export_wide.iloc[:, -14:]

    # join the df_export_wide with the apertura_secuencia dataframe

    df_export_wide_secuencia = apertura_secuencia.merge(df_export_wide_secuencia, left_on='key', right_on='key', how='left')

    # create a key_secuencia to key and SECUENCIA

    df_export_wide_secuencia['key_secuencia'] = df_export_wide_secuencia['key'].astype(str) + df_export_wide_secuencia['SECUENCIA'].astype(str)

    # create a %part divide the values of VENTA NETA (TON) per key_secuencia by the sum of VENTA NETA (TON) per key

    df_export_wide_secuencia['%part'] = df_export_wide_secuencia.groupby('key')['VENTA NETA (TON)'].apply(lambda x: x / x.sum())

    # fill the nan values with 0

    df_export_wide_secuencia.fillna(0, inplace=True)

    # drop the avg_fct column

    df_export_wide_secuencia.drop(['avg_fct'], axis=1, inplace=True)


    #  rename the columns with the forecast values

    df_export_wide_secuencia.rename(columns={'1': 'PVO_1', '2': 'PVO_2', '3': 'PVO_3', '4': 'PVO_4', '5': 'PVO_5', '6': 'PVO_6', '7': 'PVO_7', '8': 'PVO_8', '9': 'PVO_9', '10': 'PVO_10', '11': 'PVO_11', '12': 'PVO_12', '13': 'PVO_13', '14': 'PVO_14'}, inplace=True)

    # Multiplicate the columns that contain the PVO word in the name of the column by the %part column

    # Multiplicate the columns that contain the PVO word in the name of the column by the %part column

    df_export_wide_secuencia['forecast_1'] = df_export_wide_secuencia['PVO_1'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_2'] = df_export_wide_secuencia['PVO_2'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_3'] = df_export_wide_secuencia['PVO_3'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_4'] = df_export_wide_secuencia['PVO_4'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_5'] = df_export_wide_secuencia['PVO_5'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_6'] = df_export_wide_secuencia['PVO_6'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_7'] = df_export_wide_secuencia['PVO_7'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_8'] = df_export_wide_secuencia['PVO_8'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_9'] = df_export_wide_secuencia['PVO_9'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_10'] = df_export_wide_secuencia['PVO_10'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_11'] = df_export_wide_secuencia['PVO_11'] * df_export_wide_secuencia['%part']

    df_export_wide_secuencia['forecast_12'] = df_export_wide_secuencia['PVO_12'] * df_export_wide_secuencia['%part']

    # drop the columns that contain the PVO word in the name of the column

    df_export_wide_secuencia.drop(['PVO_1', 'PVO_2', 'PVO_3', 'PVO_4', 'PVO_5', 'PVO_6', 'PVO_7', 'PVO_8', 'PVO_9', 'PVO_10', 'PVO_11', 'PVO_12'], axis=1, inplace=True)

    # export the df_export_wide_secuencia dataframe to a xlsx file

    #df_export_wide_secuencia.to_excel('df_export_wide_secuencia.xlsx')

  
# save the forecast_df dataframe to a xlxs file

#df_export_wide.to_excel('df_export_wide.xlsx')

#forecast_df.to_excel('forecast_df.xlsx')

# show the df_export_wide dataframe in streamlit
...
try:
    ...
    st.subheader('Descargar datos por secuencia')

    st.markdown(get_table_download_link_xlsx(df_export_wide_secuencia), unsafe_allow_html=True)

    # estimate the diferences after the apportioenment
    
    df_export_wide['PVO_TOTAL'] = df_export_wide['PVO_1'] + df_export_wide['PVO_2'] + df_export_wide['PVO_3'] + df_export_wide['PVO_4'] + df_export_wide['PVO_5'] + df_export_wide['PVO_6'] + df_export_wide['PVO_7'] + df_export_wide['PVO_8'] + df_export_wide['PVO_9'] + df_export_wide['PVO_10'] + df_export_wide['PVO_11'] + df_export_wide['PVO_12']

        # estimate the total forecast COLUMNS

    df_export_wide_secuencia['forecast_TOTAL'] = df_export_wide_secuencia['forecast_1'] + df_export_wide_secuencia['forecast_2'] + df_export_wide_secuencia['forecast_3'] + df_export_wide_secuencia['forecast_4'] + df_export_wide_secuencia['forecast_5'] + df_export_wide_secuencia['forecast_6'] + df_export_wide_secuencia['forecast_7'] + df_export_wide_secuencia['forecast_8'] + df_export_wide_secuencia['forecast_9'] + df_export_wide_secuencia['forecast_10'] + df_export_wide_secuencia['forecast_11'] + df_export_wide_secuencia['forecast_12']
    
        # Aggregate by key  df_export_wide_secuencia and sum the values of the columns forecast_TOTAL  and rename the columns to forecast_TOTAL

    df_export_wide_secuencia_res = df_export_wide_secuencia.groupby(['key']).sum().reset_index()

    # join the df_export_wide dataframe with the df_export_wide_secuencia_res dataframe and select the columns key, PVO_TOTAL and forecast_TOTAL

    df_export_wide_secuencia_res = df_export_wide.merge(df_export_wide_secuencia_res[['key', 'forecast_TOTAL']], on='key', how='left')

    

    # estimate the difference between df_export_wide['PVO_TOTAL'] and df_export_wide_secuencia_res['forecast_TOTAL']

    df_export_wide_secuencia_res['DIFERENCIA'] = df_export_wide_secuencia_res['forecast_TOTAL'] - df_export_wide_secuencia_res['PVO_TOTAL']

    # show the sum of DIFERENCIA column IN ALERT

    st.info('Diferencia total: ' + str(df_export_wide_secuencia_res['DIFERENCIA'].sum()))


    # download the df_export_wide_secuencia_res dataframe to a xlsx file link

    st.markdown(get_table_download_link_xlsx(df_export_wide_secuencia_res), unsafe_allow_html=True)
 ...
 # estimate the total PVO COLUMNS



    # show the time it took to run the code
